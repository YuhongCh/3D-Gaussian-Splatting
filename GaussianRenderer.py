...
class GaussianRenderer(nn.Module):
    ...
    def forward(self, cam: Camera, num_tiles: int = 64) -> torch.Tensor:
        return self.paper_render(cam, num_tiles)
    ...
    def paper_render(self, cam: Camera, tile_length: int = 64):
        screen_coords = torch.zeros_like(self.model.coords, dtype=torch.float32, device=self.device, requires_grad=True)
        tanfovx = math.tan(Camera.focal2fov(cam.fx, cam.width) * 0.5)
        tanfovy = math.tan(Camera.focal2fov(cam.fy, cam.height) * 0.5)

        w, x, y, z = cam.transform.rotation
        view_matrix = cam.transform.get_model2world_matrix(use_torch=True)
        ...
        raster_settings = GaussianRasterizationSettings(
            ...
            scale_modifier=1.0,
            viewmatrix=torch.linalg.inv(view_matrix),
        ...
        # view_dir = self.model.coords - torch.tensor(cam.transform.position, dtype=torch.float32, device=self.device)
        # color = eval_sh(self.model.sh_degree,
        #                 self.model.sh,
        #                 view_dir / view_dir.norm(dim=1, keepdim=True))
        # color = torch.clamp(color + 0.5, min=0.0)
